    ...
    const crates = fs.readdirSync(subcratesFolder).map((x) => {
      if(path.extname(x) === ".crate"){ 
        ...
        const videoCrates = fs.readdirSync(path.join(subcratesFolder, x).map((elem) => {
          console.log("Found video crate with name " + x);
          const name = path.basename(x, ".crate");
          return new Crate(name, seratoFolder);
        });
        return videoCrates;
...
class Crate {
  ...
  constructor(name, seratoFolder) {
    ...
    this.name = name; // Removed sanitization for now because of incorrect file paths on song load
